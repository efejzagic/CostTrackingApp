...
const OrderHistoryPage = () => {
  ...
  const fetchOrderData = async () => {
    try {
      ...
      const response = await axios.get('http://localhost:8001/api/v/Order', getConfigHeader());
        setIsLoading(false);
    ...
    <StyledPage>
      ...
      <Container maxWidth="md" style={{ marginTop: '2rem', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', width: '100%' }}>
          ...
          </div><TableContainer component={Paper} style={{ overflowX: 'auto', minWidth: 1200, alignSelf: 'center' }}>
            ...
            <TableBody>
              {data.map((item) => (
                <TableRow key={item.id}>
                  ...
                  <TableCell>{item.orderComplete ? 'Yes' : 'No'}</TableCell>
                  <TableCell>{item.totalAmount} KM</TableCell>
                  
                  <TableCell>
                    <div style={{ display: 'flex', gap: '0.5rem' }}>
                      
                    <Button onClick={() => navigate(`/order/${item.id}/details`)} variant="outlined" color="secondary" size="small">
                        Details
                      </Button>
